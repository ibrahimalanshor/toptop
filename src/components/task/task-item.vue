...
<template>
  <div class="relative flex items-start">
    <div class="flex h-6 items-center">
      <input aria-describedby="comments-description" name="comments" type="checkbox" class="h-4 w-4 rounded-full border-gray-300 text-blue-600 focus:ring-blue-600" v-model="isDone" />
    </div>
    <div class="ml-3 text-sm leading-6 flex items-center gap-x-2 group">
      <label class="font-medium text-gray-900" :class="{ 'line-through': props.task.done }">{{ props.task.name }}</label>
      <button class="hidden group-hover:block" v-on:click="handleEdit">
    ...
    <task-edit-modal :task="editTaskModal.task" v-model="editTaskModal.visible"></task-edit-modal>
  </div>
</template>